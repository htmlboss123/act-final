...
async def pm_search(client, message):
    try:
        files, n_offset, total = await get_search_results(message.chat.id, message.text)
        if int(total) != 0:
            btn = [[InlineKeyboardButton("Here", url=FILMS_LINK)]]
            await message.reply_text(
                f"Total {total} results found in this group",
        ...
        await message.reply(f"{files}\n{n_offset}\n{ total}")
    except Exception as e:
        await message.reply(f"got this err : {e}")
    ...
    if settings["shortlink"]:
        btn.insert(
            ...
            [
                ...
                InlineKeyboardButton(
                    "🥇 ʙᴜʏ 🥇", url=f"https://t.me/{temp.U_NAME}?start=plans"
                ),
                InlineKeyboardButton(
                    "📰 ʟᴀɴɢᴜᴀɢᴇs 📰", callback_data=f"languages#{key}#{req}#{offset}"
                ),
            ],
        )
    else:
    ...
    if n_offset == 0:
        btn.append(
            [
                ...
                InlineKeyboardButton(
                    f"{math.ceil(int(offset) / MAX_BTN) + 1}/{math.ceil(total / MAX_BTN)}",
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    await query.message.edit_text(
        "<b>ɪɴ ᴡʜɪᴄʜ ʟᴀɴɢᴜᴀɢᴇ ᴅᴏ ʏᴏᴜ ᴡᴀɴᴛ, sᴇʟᴇᴄᴛ ʜᴇʀᴇ</b>",
        disable_web_page_preview=True,
        reply_markup=InlineKeyboardMarkup(btn),
    )
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    files, l_offset, total_results = await get_search_results(
        query.message.chat.id, search, lang=lang
    )
    if not files:
        await query.answer(
            f"sᴏʀʀʏ '{lang.title()}' ʟᴀɴɢᴜᴀɢᴇ ꜰɪʟᴇs ɴᴏᴛ ꜰᴏᴜɴᴅ 😕", show_alert=1
        )
        return
    temp.FILES[key] = files
    settings = await get_settings(query.message.chat.id)
    ...
    if l_offset != "":
        btn.append(
            [
                ...
                InlineKeyboardButton(
                    text="ɴᴇxᴛ »",
                    callback_data=f"lang_next#{req}#{key}#{lang}#{l_offset}#{offset}",
                ),
            ]
        )
    else:
...
async def lang_next_page(bot, query):
    ...
    files, n_offset, total = await get_search_results(
        query.message.chat.id, search, offset=l_offset, lang=lang
    )
    if not files:
        return
    temp.FILES[key] = files
    ...
    if n_offset == 0:
        btn.append(
            [
                InlineKeyboardButton(
                    "« ʙᴀᴄᴋ",
                    callback_data=f"lang_next#{req}#{key}#{lang}#{b_offset}#{offset}",
                ),
                InlineKeyboardButton(
                    f"{math.ceil(int(l_offset) / MAX_BTN) + 1}/{math.ceil(total / MAX_BTN)}",
                    callback_data="buttons",
                ),
            ]
        )
        ...
        btn.append(
            [
                ...
                InlineKeyboardButton(
                    "ɴᴇxᴛ »",
                    callback_data=f"lang_next#{req}#{key}#{lang}#{n_offset}#{offset}",
                ),
            ]
        )
    else:
...
async def advantage_spoll_choker(bot, query):
    ...
    files, offset, total_results = await get_search_results(bot.chat.id, query, search)
    if files:
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [
            ...
            [
                InlineKeyboardButton(
                    "🔎 sᴇᴀʀᴄʜ ɪɴʟɪɴᴇ 🔍", switch_inline_query_current_chat=""
                )
            ],
            [
        ...
        buttons = [[InlineKeyboardButton("« ʙᴀᴄᴋ", callback_data="my_about")]]
        await query.message.edit_text(
        ...
        if settings is not None:
            buttons = [
                ...
                [
                    InlineKeyboardButton(
                        "Fsub",
                        callback_data=f'setgs#is_fsub#{settings.get("is_fsub", IS_FSUB)}#{str(grp_id)}',
                    ),
                    InlineKeyboardButton(
                        "✅ On" if settings.get("is_fsub", IS_FSUB) else "❌ Off",
                        callback_data=f'setgs#is_fsub#{settings.get("is_fsub", IS_FSUB)}#{str(grp_id)}',
                    ),
                ],
                [
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        settings = await get_settings(message.chat.id)
        search = message.text
        files, offset, total_results = await get_search_results(msg.chat.id, search)
        if not files:
        ...
        if settings["shortlink"]:
            btn.insert(
                ...
                [
                    ...
                    InlineKeyboardButton(
                        "📰 ʟᴀɴɢᴜᴀɢᴇs 📰", callback_data=f"languages#{key}#{req}#0"
                    ),
                ],
            )
        else:
            btn.insert(
                ...
                [
                    InlineKeyboardButton(
                        "♻️ sᴇɴᴅ ᴀʟʟ ♻️", callback_data=f"send_all#{key}"
                    ),
                    InlineKeyboardButton(
                        "🥇 ʙᴜʏ 🥇", url=f"https://t.me/{temp.U_NAME}?start=plans"
                    ),
...
async def advantage_spell_chok(message):
    ...
    btn = [
        [
            ...
            InlineKeyboardButton(
                "🔎 Search Google 🔍",
                url=f"https://www.google.com/search?q={google_search}",
            ),
        ]
    ]
    try:
